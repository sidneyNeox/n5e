/**
 * The Naruto5e game system for Foundry Virtual Tabletop
 * A system for playing the fifth edition of the worlds most popular roleplaying game.
 * Author: Yaco, System made by Kingsare4ever
 */

// Import Modules
import { N5E } from "./module/config.js";
...
Hooks.once("init", function() {
  console.log(`N5e | Initializing the N5e Game System\n${N5E.ASCII}`);
  ...
  // This will be removed when n5e minimum core version is updated to v9.
  ...
  Actors.registerSheet("n5e", ActorSheet5eCharacter, {
    types: ["character"],
    makeDefault: true,
    label: "N5E.SheetClassCharacter"
  });
  Actors.registerSheet("n5e", ActorSheet5eNPC, {
    types: ["npc"],
    makeDefault: true,
    label: "N5E.SheetClassNPC"
  });
  Actors.registerSheet("n5e", ActorSheet5eVehicle, {
    types: ["vehicle"],
    makeDefault: true,
    label: "N5E.SheetClassVehicle"
  });
  Items.unregisterSheet("core", ItemSheet);
  Items.registerSheet("n5e", ItemSheet5e, {
    makeDefault: true,
...
Hooks.once("setup", function() {
  ...
  preLocalizeConfig(CONFIG.N5E, localizeKeys, sortKeys);
  CONFIG.N5E.trackableAttributes = expandAttributeList(CONFIG.N5E.trackableAttributes);
  CONFIG.N5E.consumableResources = expandAttributeList(CONFIG.N5E.consumableResources);
});
...
Hooks.once("ready", function() {
  ...
  const currentVersion = game.settings.get("n5e", "systemMigrationVersion");
  const NEEDS_MIGRATION_VERSION = "1.5.5";
  const COMPATIBLE_MIGRATION_VERSION = 0.80;
  const totalDocuments = game.actors.size + game.scenes.size + game.items.size;
  if ( !currentVersion && totalDocuments === 0 ) return game.settings.set("n5e", "systemMigrationVersion", game.system.data.version);
  const needsMigration = !currentVersion || isNewerVersion(NEEDS_MIGRATION_VERSION, currentVersion);
  if ( !needsMigration ) return;

  // Perform the migration
  if ( currentVersion && isNewerVersion(COMPATIBLE_MIGRATION_VERSION, currentVersion) ) {
    const warning = "Your N5e system data is from too old a Foundry version and cannot be reliably migrated to the latest version. The process will be attempted, but errors may occur.";
...
Hooks.on("canvasInit", function() {
  ...
  canvas.grid.diagonalRule = game.settings.get("n5e", "diagonalMovement");
...
Hooks.on("renderChatMessage", (app, html, data) => {
  ...
  if (game.settings.get("n5e", "autoCollapseItemCards")) html.find(".card-content").hide();
});
