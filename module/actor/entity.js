...
export default class Actor5e extends Actor {
   ...
   */
  get isPolymorphed() {
    return this.getFlag("n5e", "isPolymorphed") || false;
  }
  ...
  prepareDerivedData() {
    ...
    const flags = actorData.flags.n5e || {};
    ...
    if (this.isPolymorphed) {
      const transformOptions = this.getFlag("n5e", "transformOptions");
      const original = game.actors?.get(this.getFlag("n5e", "originalActor"));
      if (original) {
   ...
   */
  getLevelExp(level) {
    const levels = CONFIG.N5E.CHARACTER_EXP_LEVELS;
  ...
  getCRExp(cr) {
    ...
    return CONFIG.N5E.CR_EXP_LEVELS[cr];
  }
  ...
  async addEmbeddedItems(items, prompt=true) {
    ...
    if (prompt) {
      const itemIdsToAdd = await SelectItemsPrompt.create(items, {
        hint: game.i18n.localize("N5E.AddEmbeddedItemPromptHint")
      });
  ...
  static async loadClassFeatures({className="", subclassName="", level=1, priorLevel=0}={}) {
    ...
    const clsConfig = CONFIG.N5E.classFeatures[className];
    if (!clsConfig) return [];
  ...
  _prepareCharacterData(actorData) {
    ...
    const [level, hd, cd] = this.items.reduce((arr, item) => {
      if ( item.type === "class" ) {
        const classLevels = parseInt(item.data.data.levels) || 1;
        arr[0] += classLevels;
        arr[1] += classLevels - (parseInt(item.data.data.hitDiceUsed) || 0);
        arr[2] += classLevels - (parseInt(item.data.data.chakraDiceUsed) || 0);
      }
      return arr;
    }, [0, 0]);
    data.details.level = level;
    data.attributes.hd = hd;
    data.attributes.cd = cd;

    // Character proficiency bonus
    data.attributes.prof = Math.floor((level + 8) / 3);
  ...
  _prepareNPCData(actorData) {
    ...
    // Proficiency
    data.attributes.prof = Math.floor((Math.max(data.details.cr, 1) + 8) / 3);
  ...
  _prepareSkills(actorData, bonusData, bonuses, checkBonus, originalSkills) {
    ...
    // Skill modifiers
    const feats = N5E.characterFlags;
    const joat = flags.jackOfAllTrades;
  ...
  _computeInitiativeModifier(actorData, globalCheckBonus, bonusData) {
    ...
    init.mod = data.abilities.dex.mod + Math.floor(data.attributes.prof / 2);
  ...
  _computeSpellcastingProgression(actorData) {
    ...
    const slots = N5E.SPELL_SLOT_TABLE[levels - 1] || [];
  ...
  _computeArmorClass(data) {
    ...
    ac.warnings = [];
    let cfg = CONFIG.N5E.armorClasses[ac.calc];
    if ( !cfg ) {
      ac.calc = "flat";
      if ( Number.isNumeric(ac.value) ) ac.flat = Number(ac.value);
      cfg = CONFIG.N5E.armorClasses.flat;
    }

    // Identify Equipped Items
    const armorTypes = new Set(Object.keys(CONFIG.N5E.armorTypes));
    ...
    switch ( ac.calc ) {
      ...
      case "default":
        if ( armors.length ) {
          if ( armors.length > 1 ) ac.warnings.push("N5E.WarnMultipleArmor");
      ...
      default:
        ...
        try {
          ...
          ac.warnings.push("N5E.WarnBadACFormula");
          const replaced = Roll.replaceFormulaData(CONFIG.N5E.armorClasses.default.formula, rollData);
          ac.base = Roll.safeEval(replaced);
        }
        break;
    }
    ...
    // Equipped Shield
    if ( shields.length ) {
      if ( shields.length > 1 ) ac.warnings.push("N5E.WarnMultipleShields");
  ...
  _computeEncumbrance(actorData) {
    ...
    if ( game.settings.get("n5e", "currencyWeight") && actorData.data.currency ) {
      const currency = actorData.data.currency;
      const numCoins = Object.values(currency).reduce((val, denom) => val += Math.max(denom, 0), 0);

      const currencyPerWeight = game.settings.get("n5e", "metricWeightUnits")
        ? CONFIG.N5E.encumbrance.currencyPerWeight.metric
        : CONFIG.N5E.encumbrance.currencyPerWeight.imperial;
    ...
    if ( this.getFlag("n5e", "powerfulBuild") ) mod = Math.min(mod * 2, 8);

    // Compute Encumbrance percentage
    weight = weight.toNearest(0.1);

    const strengthMultiplier = game.settings.get("n5e", "metricWeightUnits")
      ? CONFIG.N5E.encumbrance.strMultiplier.metric
      : CONFIG.N5E.encumbrance.strMultiplier.imperial;
  ...
  async _preCreate(data, options, user) {
    ...
    // Token size category
    const s = CONFIG.N5E.tokenSizes[this.data.data.traits.size || "med"];
  ...
  async _preUpdate(changed, options, user) {
    ...
    if ( newSize && (newSize !== foundry.utils.getProperty(this.data, "data.traits.size")) ) {
      let size = CONFIG.N5E.tokenSizes[newSize];
  ...
  async applyDamage(amount=0, multiplier=1) {
    ...
    return allowed !== false ? this.update(updates, {dhp: -amount}) : this;
      ...
      async applyChakraDamage(amount=0, multiplier=1) {
    amount = Math.floor(parseInt(amount) * multiplier);
    const ch = this.data.data.attributes.ch;

    // Deduct damage from temp CH first
    const ctmp = parseInt(ch.temp) || 0;
    const dct = amount > 0 ? Math.min(ctmp, amount) : 0;

    // Remaining goes to chakra
    const ctmpMax = parseInt(ch.tempmax) || 0;
    const dc = Math.clamped(ch.value - (amount - dct), 0, ch.max + ctmpMax);

    // Update the Actor
    const updates = {
      "data.attributes.ch.temp": ctmp - dct,
      "data.attributes.ch.value": dc
    };

    // Delegate damage application to a hook
    // TODO replace this in the future with a better modifyTokenAttribute function in the core
    const allowed = Hooks.call("modifyTokenAttribute", {
      attribute: "attributes.ch",
      value: amount,
      isDelta: false,
      isBar: true
    }, updates);
    return allowed !== false ? this.update(updates, {dcp: -amount}) : this;
   }
  ...
  _isRemarkableAthlete(ability) {
    return this.getFlag("n5e", "remarkableAthlete") && N5E.characterFlags.remarkableAthlete.abilities.includes(ability);
  }
  ...
  rollSkill(skillId, options={}) {
    ...
    const reliableTalent = (skl.value >= 1 && this.getFlag("n5e", "reliableTalent"));

    // Roll and return
    const rollData = foundry.utils.mergeObject(options, {
      parts: parts,
      data: data,
      title: `${game.i18n.format("N5E.SkillPromptTitle", {skill: CONFIG.N5E.skills[skillId]})}: ${this.name}`,
      halflingLucky: this.getFlag("n5e", "halflingLucky"),
      reliableTalent: reliableTalent,
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.n5e.roll": {type: "skill", skillId }
      }
    });
  ...
  rollAbility(abilityId, options={}) {
    const label = CONFIG.N5E.abilities[abilityId];
    new Dialog({
      title: `${game.i18n.format("N5E.AbilityPromptTitle", {ability: label})}: ${this.name}`,
      content: `<p>${game.i18n.format("N5E.AbilityPromptText", {ability: label})}</p>`,
      buttons: {
        test: {
          label: game.i18n.localize("N5E.ActionAbil"),
          callback: () => this.rollAbilityTest(abilityId, options)
        },
        save: {
          label: game.i18n.localize("N5E.ActionSave"),
  ...
  rollAbilityTest(abilityId, options={}) {
    ...
    const rollData = foundry.utils.mergeObject(options, {
      ...
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.n5e.roll": {type: "ability", abilityId }
      }
    });
    return d20Roll(rollData);
  }
  ...
  rollAbilitySave(abilityId, options={}) {
    ...
    const rollData = foundry.utils.mergeObject(options, {
      parts: parts,
      data: data,
      title: `${game.i18n.format("N5E.SavePromptTitle", {ability: label})}: ${this.name}`,
      halflingLucky: this.getFlag("n5e", "halflingLucky"),
      messageData: {
        speaker: options.speaker || ChatMessage.getSpeaker({actor: this}),
        "flags.n5e.roll": {type: "save", abilityId }
      }
    });
    return d20Roll(rollData);
  }
  ...
  async rollDeathSave(options={}) {
    ...
    if ( (this.data.data.attributes.hp.value > 0) || (death.failure >= 3) || (death.success >= 3)) {
      ui.notifications.warn(game.i18n.localize("N5E.DeathSaveUnnecessary"));
      return null;
    }
    ...
    if ( this.getFlag("n5e", "diamondSoul") ) {
      parts.push("@prof");
    ...
    const rollData = foundry.utils.mergeObject(options, {
      parts: parts,
      data: data,
      title: `${game.i18n.localize("N5E.DeathSavingThrow")}: ${this.name}`,
      halflingLucky: this.getFlag("n5e", "halflingLucky"),
      targetValue: 10,
      messageData: {
        speaker: speaker,
        "flags.n5e.roll": {type: "death"}
      }
    });
    ...
    if ( success ) {
      ...
      if ( d20 === 20 ) {
        ...
        chatString = "N5E.DeathSaveCriticalSuccess";
      }
      ...
      // 3 Successes = survive and reset checks
        ...
        chatString = "N5E.DeathSaveSuccess";
      }
    ...
    // Save failure
      ...
      if ( failures >= 3 ) {  // 3 Failures = death
        chatString = "N5E.DeathSaveFailure";
      }
    }
  ...
  async rollHitDie(denomination, {dialog=true}={}) {
    ...
    if ( !cls ) {
      ui.notifications.error(game.i18n.format("N5E.HitDiceWarn", {name: this.name, formula: denomination}));
      return null;
    }

    // Prepare roll data
    const parts = [`1${denomination}`, "@abilities.con.mod"];
    const title = `${game.i18n.localize("N5E.HitDiceRoll")}: ${this.name}`;
    ...
    await cls.update({"data.chakraDiceUsed": cls.data.data.chakraDiceUsed + 1});
    const hp = this.data.data.attributes.hp;
    const dhp = Math.min(hp.max + (hp.tempmax ?? 0) - hp.value, roll.total);
    const cp = this.data.data.attributes.cp;
    const dcp = Math.min(cp.max + (cp.tempmax ?? 0) - cp.value, roll.total);
    const cd = this.data.data.attributes.cd;
    await this.update({"data.attributes.hp.value": hp.value + dhp});
    await this.update({"data.attributes.cp.value": cp.value + dcp});
    await this.update({"data.attributes.cd.value": cd.value + dcd});
   ...
   * @property {number} dcd            Chakra dice recovered or spent during the rest.
  ...
  async _displayRestResultMessage(result, longRest=false) {
    ...
    switch (game.settings.get("n5e", "restVariant")) {
      case "normal": restFlavor = (longRest && newDay) ? "N5E.LongRestOvernight" : `N5E.${length}RestNormal`; break;
      case "gritty": restFlavor = (!longRest && newDay) ? "N5E.ShortRestOvernight" : `N5E.${length}RestGritty`; break;
      case "epic": restFlavor = `N5E.${length}RestEpic`; break;
    }

    // Determine the chat message to display
    if ( diceRestored && healthRestored ) message = `N5E.${length}RestResult`;
    else if ( longRest && !diceRestored && healthRestored ) message = "N5E.LongRestResultHitPoints";
    else if ( longRest && diceRestored && !healthRestored ) message = "N5E.LongRestResultHitDice";
    else message = `N5E.${length}RestResultShort`;
  ...
  convertCurrency() {
    ...
    const conversion = Object.entries(CONFIG.N5E.currencies);
    conversion.reverse();
  ...
  async transformInto(target, { keepPhysical=false, keepMental=false, keepSaves=false, keepSkills=false,
    ...
    const allowed = game.settings.get("n5e", "allowPolymorphing");
    if ( !allowed && !game.user.isGM ) {
      return ui.notifications.warn(game.i18n.localize("N5E.PolymorphWarn"));
    }

    // Get the original Actor data and the new source data
    const o = this.toJSON();
    o.flags.n5e = o.flags.n5e || {};
    o.flags.n5e.transformOptions = {mergeSkills, mergeSaves};
    ...
    if (!keepClass && d.data.details.cr) {
      d.items.push({
        type: "class",
        name: game.i18n.localize("N5E.PolymorphTmpClass"),
    ...
    // Set new data flags
    if ( !this.isPolymorphed || !d.flags.n5e.originalActor ) d.flags.n5e.originalActor = this.id;
    d.flags.n5e.isPolymorphed = true;
    ...
    Hooks.callAll("n5e.transformActor", this, target, d, {
  ...
  async revertOriginalForm() {
    ...
    if ( !this.isOwner ) {
      return ui.notifications.warn(game.i18n.localize("N5E.PolymorphRevertWarn"));
    }
  ...
  static addDirectoryContextOptions(html, entryOptions) {
    entryOptions.push({
      name: "N5E.PolymorphRestoreTransformation",
  ...
  static formatCreatureType(typeData) {
    ...
    if ( typeData.value === "custom" ) {
      ...
      let code = CONFIG.N5E.creatureTypes[typeData.value];
      localizedType = game.i18n.localize(typeData.swarm ? `${code}Pl` : code);
    }
    let type = localizedType;
    if ( typeData.swarm ) {
      type = game.i18n.format("N5E.CreatureSwarmPhrase", {
        size: game.i18n.localize(CONFIG.N5E.actorSizes[typeData.swarm]),
        type: localizedType
      });
    }
  ...
  static prepareProficiencies(data, type) {
    const profs = CONFIG.N5E[`${type}Proficiencies`];
    const itemTypes = CONFIG.N5E[`${type}Ids`];
    ...
    for ( const key of values ) {
      ...
      } else if ( type === "tool" && CONFIG.N5E.vehicleTypes[key] ) {
        data.selected[key] = CONFIG.N5E.vehicleTypes[key];
      }
    }
  ...
  _displayScrollingDamage(dhp) {
    ...
    for ( let t of tokens ) {
      ...
      t.hud.createScrollingText(dhp.signedString(), {
        ...
        fill: CONFIG.N5E.tokenHPColors[dhp < 0 ? "damage" : "healing"],
        stroke: 0x000000,
        strokeThickness: 4,
        jitter: 0.25
      });
   ...
   * @param {string} ability  Ability key as defined in `CONFIG.N5E.abilities`.
   * @returns {number}        Spell save DC for provided ability.
   * @deprecated since n5e 0.97
   */
  getSpellDC(ability) {
