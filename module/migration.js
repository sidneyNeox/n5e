...
export const migrateWorld = async function() {
  ui.notifications.info(`Applying N5E System Migration for version ${game.system.data.version}. Please be patient and do not close your game or shut down your server.`, {permanent: true});
  ...
  for ( let a of game.actors ) {
    try {
      ...
      err.message = `Failed n5e system migration for Actor ${a.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( let i of game.items ) {
    try {
      ...
      err.message = `Failed n5e system migration for Item ${i.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( const m of game.macros ) {
    try {
      ...
      err.message = `Failed n5e system migration for Macro ${m.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  for ( let s of game.scenes ) {
    try {
      ...
      err.message = `Failed n5e system migration for Scene ${s.name}: ${err.message}`;
      console.error(err);
    }
  }
  ...
  game.settings.set("n5e", "systemMigrationVersion", game.system.data.version);
  ui.notifications.info(`N5E System Migration to version ${game.system.data.version} completed!`, {permanent: true});
};
...
export const migrateCompendium = async function(pack) {
  ...
  for ( let doc of documents ) {
    ...
    // Handle migration failures
      ...
      err.message = `Failed n5e system migration for entity ${doc.name} in pack ${pack.collection}: ${err.message}`;
      console.error(err);
    }
  }
...
export const migrateArmorClass = async function(pack) {
  ...
  const updates = [];
  const armor = new Set(Object.keys(CONFIG.N5E.armorTypes));
...
function cleanActorData(actorData) {
  ...
  // Scrub system flags
  const allowedFlags = CONFIG.N5E.allowedActorFlags.reduce((obj, f) => {
    obj[f] = null;
    return obj;
  }, {});
  if ( actorData.flags.n5e ) {
    actorData.flags.n5e = filterObject(actorData.flags.n5e, allowedFlags);
  }
...
export const getMigrationData = async function() {
  const data = {};
  try {
    let res = await fetch("systems/n5e/json/icon-migration.json");
    data.iconMap = await res.json();
    res = await fetch("systems/n5e/json/undo-icons.json");
...
function _migrateActorType(actor, updateData) {
  ...
  if ( match ) {
    ...
    // Match a swarm
    const isNamedSwarm = actor.name.startsWith(game.i18n.localize("N5E.CreatureSwarm"));
...
const TOKEN_IMAGE_RENAME = {
  "systems/n5e/tokens/beast/OwlWhite.png": "systems/n5e/tokens/beast/Owl.webp",
  "systems/n5e/tokens/beast/ScorpionSand.png": "systems/n5e/tokens/beast/Scorpion.webp",
  "systems/n5e/tokens/beast/BaboonBlack.png": "systems/n5e/tokens/beast/Baboon.webp",
  "systems/n5e/tokens/humanoid/BanditRedM.png": "systems/n5e/tokens/humanoid/Bandit.webp",
  "systems/n5e/tokens/humanoid/GuardBlueM.png": "systems/n5e/tokens/humanoid/Guard.webp",
  "systems/n5e/tokens/humanoid/HumanBrownM.png": "systems/n5e/tokens/humanoid/Commoner.webp",
  "systems/n5e/tokens/humanoid/NobleSwordM.png": "systems/n5e/tokens/humanoid/Noble.webp",
  "systems/n5e/tokens/humanoid/MerfolkBlue.png": "systems/n5e/tokens/humanoid/Merfolk.webp",
  "systems/n5e/tokens/humanoid/TribalWarriorM.png": "systems/n5e/tokens/humanoid/TribalWarrior.webp",
  "systems/n5e/tokens/devil/Lemure.png": "systems/n5e/tokens/fiend/Lemure.webp",
  "systems/n5e/tokens/humanoid/Satyr.png": "systems/n5e/tokens/fey/Satyr.webp",
  "systems/n5e/tokens/beast/WinterWolf.png": "systems/n5e/tokens/monstrosity/WinterWolf.webp"
};
...
const TOKEN_IMAGE_RESCALE = {
  "systems/n5e/tokens/beast/HunterShark.png": 1.5,
  "systems/n5e/tokens/beast/GiantElk.png": 1.5,
  "systems/n5e/tokens/monstrosity/Bulette.png": 1.5,
  "systems/n5e/tokens/beast/Wolf.png": 1.5,
  "systems/n5e/tokens/beast/Panther.png": 1.5,
  "systems/n5e/tokens/beast/Elk.png": 1.5,
  "systems/n5e/tokens/beast/AxeBeak.png": 1.5,
  "systems/n5e/tokens/beast/GiantVulture.png": 1.5,
  "systems/n5e/tokens/beast/GiantSpider.png": 1.5,
  "systems/n5e/tokens/beast/DireWolf.png": 1.5,
  "systems/n5e/tokens/monstrosity/DeathDog.png": 1.5,
  "systems/n5e/tokens/devil/Lemure.png": 1.5,
  "systems/n5e/tokens/beast/Deer.png": 1.1,
  "systems/n5e/tokens/beast/GiantWeasel.png": 1.5,
  "systems/n5e/tokens/beast/Camel.png": 1.2,
  "systems/n5e/tokens/beast/BloodHawk.png": 1.5
};
...
function _migrateTokenImage(actorData, updateData) {
  ["img", "token.img"].forEach(prop => {
    ...
    if ( img === "systems/n5e/tokens/humanoid/HumanBrownM.webp" ) {
      updateData[prop] = "systems/n5e/tokens/humanoid/Commoner.webp";
      return updateData;
    }
    if ( !img?.startsWith("systems/n5e/tokens/") || img?.endsWith(".webp") ) return;
...
function _migrateItemAttunement(item, updateData) {
  ...
  updateData["data.attunement"] = CONFIG.N5E.attunementTypes.NONE;
...
function _migrateItemRarity(item, updateData) {
  ...
  const rarity = Object.keys(CONFIG.N5E.itemRarity).find(key =>
    (CONFIG.N5E.itemRarity[key].toLowerCase() === item.data.rarity.toLowerCase()) || (key === item.data.rarity)
  );
...
function _migrateItemIcon(item, updateData, {iconMap, undoMap}={}) {
  if ( iconMap && item.img?.startsWith("systems/n5e/icons/") ) {
...
export async function purgeFlags(pack) {
  const cleanFlags = flags => {
    const flags5e = flags.n5e || null;
    return flags5e ? {n5e: flags5e} : {};
  };
  await pack.configure({locked: false});
