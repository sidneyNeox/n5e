...
export default class AbilityUseDialog extends Dialog {
  ...
  static async create(item) {
    ...
    const data = {
      item: item.data,
      title: game.i18n.format("N5E.AbilityUseHint", {type: game.i18n.localize(`N5E.ItemType${item.type.capitalize()}`), name: item.name}),
    ...
    const html = await renderTemplate("systems/n5e/templates/apps/ability-use.html", data);

    // Create the Dialog and return data as a Promise
    const icon = data.isSpell ? "fa-magic" : "fa-fist-raised";
    const label = game.i18n.localize(`N5E.AbilityUse${data.isSpell ? "Cast" : "Use"}`);
  ...
  static _getSpellData(actorData, itemData, data) {
    ...
    const consumeSpellSlot = (lvl > 0) && CONFIG.N5E.spellUpcastModes.includes(itemData.preparation.mode);
    ...
    const spellLevels = Array.fromRange(10).reduce((arr, i) => {
      if ( i < lvl ) return arr;
      const label = CONFIG.N5E.spellLevels[i];
      ...
      arr.push({
        level: i,
        label: i > 0 ? game.i18n.format("N5E.SpellLevelSlot", {level: label, n: slots}) : label,
        canCast: max > 0,
        hasSlots: slots > 0
      });
      return arr;
    ...
    if (pact.level >= lvl) {
      spellLevels.push({
        level: "pact",
        label: `${game.i18n.format("N5E.SpellLevelPact", {level: pact.level, n: pact.value})}`,
        canCast: true,
        hasSlots: pact.value > 0
      });
    }
    const canCast = spellLevels.some(l => l.hasSlots);
    if ( !canCast ) data.errors.push(game.i18n.format("N5E.SpellCastNoSlots", {
      level: CONFIG.N5E.spellLevels[lvl],
      name: data.item.name
    }));
  ...
  static _getAbilityUseNote(item, uses, recharge) {
    ...
    if ( recharge.value ) {
      return game.i18n.format(recharge.charged ? "N5E.AbilityUseChargedHint" : "N5E.AbilityUseRechargeHint", {
        type: game.i18n.localize(`N5E.ItemType${item.type.capitalize()}`)
      });
    }
    ...
    if ( item.type === "consumable" ) {
      ...
      else if ( item.data.quantity === 1 && uses.autoDestroy ) str = "N5E.AbilityUseConsumableDestroyHint";
      else if ( item.data.quantity > 1 ) str = "N5E.AbilityUseConsumableQuantityHint";
      return game.i18n.format(str, {
        type: game.i18n.localize(`N5E.Consumable${item.data.consumableType.capitalize()}`),
        value: uses.value,
