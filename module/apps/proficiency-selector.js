...
export default class ProficiencySelector extends TraitSelector {
  ...
  static async getChoices(type, chosen=[]) {
    let data = Object.entries(CONFIG.N5E[`${type}Proficiencies`]).reduce((obj, [key, label]) => {
      obj[key] = { label: label, chosen: chosen.includes(key) };
      return obj;
    }, {});

    const ids = CONFIG.N5E[`${type}Ids`];
    const map = CONFIG.N5E[`${type}ProficienciesMap`];
    if ( ids !== undefined ) {
    ...
    if ( type === "tool" ) {
      data.vehicle.children = Object.entries(CONFIG.N5E.vehicleTypes).reduce((obj, [key, label]) => {
        obj[key] = { label: label, chosen: chosen.includes(key) };
        return obj;
      }, {});
  ...
  static getBaseItem(identifier, { indexOnly=false, fullItem=false }={}) {
    let pack = CONFIG.N5E.sourcePacks.ITEMS;
