...
export default class ShortRestDialog extends Dialog {
  ...
  static get defaultOptions() {
    return mergeObject(super.defaultOptions, {
      template: "systems/n5e/templates/apps/short-rest.html",
      classes: ["n5e", "dialog"]
    });
  }
  ...
  getData() {
    ...
    // Determine Chakra Dice
    data.availableCD = this.actor.data.items.reduce((cd, item) => {
      if ( item.type === "class" ) {
        const d = item.data.data;
        const denom = d.chakraDice || "d6";
        const available = parseInt(d.levels || 1) - parseInt(d.chakraDiceUsed || 0);
        cd[denom] = denom in cd ? cd[denom] + available : available;
      }
      return cd;
    }, {});
    data.canRoll = this.actor.data.data.attributes.cd > 0;
    data.denomination = this._denom;

    // Determine rest type
    const variant = game.settings.get("n5e", "restVariant");
  ...
  async _onRollHitDie(event) {
   ...
   async _onRollChakraDie(event) {
    event.preventDefault();
    const btn = event.currentTarget;
    this._denom = btn.form.cd.value;
    await this.actor.rollChakraDie(this._denom);
    this.render();
  }
  ...
  static async shortRestDialog({ actor }={}) {
    return new Promise((resolve, reject) => {
      const dlg = new this(actor, {
        title: `${game.i18n.localize("N5E.ShortRest")}: ${actor.name}`,
        buttons: {
          rest: {
            icon: '<i class="fas fa-bed"></i>',
            label: game.i18n.localize("N5E.Rest"),
            callback: html => {
              let newDay = false;
              if (game.settings.get("n5e", "restVariant") === "gritty") {
